param($installPath, $toolsPath, $package, $project)
    ...
    $target = $msbuild.Xml.AddTarget("EnsureOctoPackPlusCoreImported")
    $target.BeforeTargets = "BeforeBuild"
    $target.Condition = "'`$(OctoPackPlus_Core_Imported)' == ''"
    ...
    $errorTask.SetParameter("Text", "You are trying to build with OctoPackPlus.Core, but the NuGet targets file that it depends on is not available on this computer. This is probably because the OctoPackPlus.Core package has not been committed to source control, or NuGet Package Restore is not enabled. Please enable NuGet Package Restore to download them. For more information, see http://go.microsoft.com/fwlink/?LinkID=317567.");
    ...
    $errorTask = $target.AddTask("Error")
    $errorTask.Condition = "Exists('$relativePath')"
    $errorTask.SetParameter("Text", "OctoPackPlus.Core cannot be run because NuGet packages were restored prior to the build running, and the targets file was unavailable when the build started. Please build the project again to include these packages in the build. You may also need to make sure that your build server does not delete packages prior to each build. For more information, see http://go.microsoft.com/fwlink/?LinkID=317568.");
